...
def draw_joy((cx,cy),r,(x,y),col):
    ...
    text = [font.render('x = %.6f'%(x),12,light_gray),font.render('y = %.6f'%(y),12,light_gray),font.render('r = %.6f'%(math.sqrt(x*x+y*y)),12,light_gray),font.render('t = %.6f'%(math.atan2(-y,x)),12,light_gray)] #,font.render(str(int((math.atan2(-y,x)/math.pi+1)*127)),12,red)]
